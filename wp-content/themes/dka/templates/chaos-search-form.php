...
global $facets;
$facets = array(
	WPANPSearch::QUERY_KEY_ORGANIZATION => WPChaosSearch::generate_facet("FIATIFTA-ANP-Publisher", WPANPSearch::QUERY_KEY_ORGANIZATION)
);
...
foreach(WPANPSearch::get_organizations_merged() as $id => $organization) :
	$count = get_facet_count(WPANPSearch::QUERY_KEY_ORGANIZATION, $organization['chaos_titles']);

?>
			<label for="<?php echo WPANPSearch::QUERY_KEY_ORGANIZATION .'-'. $organization['slug']; ?>" class="btn filter-btn filter-btn-single">
				<input type="checkbox" class="chaos-filter" style="display: none;" name="<?php echo WPANPSearch::QUERY_KEY_ORGANIZATION; ?>[]" value="<?php echo $organization['slug']; ?>" id="<?php echo WPANPSearch::QUERY_KEY_ORGANIZATION .'-'. $organization['slug']; ?>" <?php checked(in_array($organization['slug'],(array)$organizations)); ?>>
